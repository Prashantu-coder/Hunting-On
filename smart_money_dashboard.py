...
st.set\_page\_config(page\_title="Quantexo", layout="wide")

with open("styles.css") as f:
st.markdown(f"<style>{f.read()}</style>", unsafe\_allow\_html=True)
...
with col1:
selected\_dropdown = st.selectbox("",options=\[""] + sorted(list(companies)), index=0, label\_visibility="collapsed")
with col2:
user\_input = st.text\_input("🔍 Enter Company Symbol","", label\_visibility="collapsed",placeholder="🔍 Enter Company Symbol")
with col3: search\_clicked = st.button("Search")

# --- Priority: Manual Entry overrides Dropdown ---

if search\_clicked:
if user\_input.strip():
company\_symbol = user\_input.strip().upper()
elif selected\_dropdown:
company\_symbol = selected\_dropdown
else:
...
def get\_sheet\_data(symbol, sheet\_name="Daily Price"):
try:
\# Google Sheets URL with the specific sheet's gid
sheet\_url = f"[https://docs.google.com/spreadsheets/d/1Q\_En7VGGfifDmn5xuiF-t\_02doPpwl4PLzxb4TBCW0Q/export?format=csv\&gid={get\_sheet\_gid(sheet\_name)}](https://docs.google.com/spreadsheets/d/1Q_En7VGGfifDmn5xuiF-t_02doPpwl4PLzxb4TBCW0Q/export?format=csv&gid={get_sheet_gid%28sheet_name%29})"
